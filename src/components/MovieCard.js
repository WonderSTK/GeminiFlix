...
const MovieCard = ({ movie }) => {
    ...
    const releaseDate = movie.release_date
        ? format(new Date(movie.release_date), 'MMMM dd, yyyy') // Using date-fns for better formatting
        : "Unknown Release Date";

    return (
        <div
            className="w-32 sm:w-40 md:w-48 mx-2 mt-2 hover:-translate-y-3 hover:scale-105 transition-all cursor-pointer"
            onClick={handleMovieInfoPage}
        >
            <div className="bg-black bg-opacity-70 rounded-lg overflow-hidden relative">
                <img
                    className="object-cover w-full h-52 sm:h-64 md:h-72 rounded-lg"
